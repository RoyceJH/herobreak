...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__gold_handler_js__ = __webpack_require__(9);
...
class Game {
  ...
  bindButtons() {
    $('.buy-breaker').on('click', this.buyBreaker);
    $('.buy-upgrade').on('click', this.upgrade);
  }
  ...
  addBreaker() {
    const breaker = new __WEBPACK_IMPORTED_MODULE_0__breaker_js__["a" /* default */]({
      ...
      pos: this.startPos,
      color: '#cb1414',
      side: __WEBPACK_IMPORTED_MODULE_5__util_js__["b" /* defaults */].breakerSide,
      skill: [],
      game: this,
    });
  ...
  checkCollisions() {
    this.currentAttackers.forEach(breaker => {
      ...
      if(breaker.pos[0] > this.DIM_X - __WEBPACK_IMPORTED_MODULE_5__util_js__["b" /* defaults */].breakerSide) {
        hitRight = true;
      }
  ...
  turnOver() {
    ...
    __WEBPACK_IMPORTED_MODULE_5__util_js__["a" /* bindKeyHandlers */](this);
    this.resetBreakers();
    this.pointer.reset();
...
class Monster {
  ...
  xRange() {
    ...
    const rightBound = this.pos[0] + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].side + 7;
    return [leftBound, rightBound];
  }

  yRange() {
    const topBound = this.pos[1] + 7;
    const bottomBound = this.pos[1]+ __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].side + 7;
  ...
  collidedBottom(breaker) {
    ...
    if( (this.yRange()[1] < breakPos[1]) && (this.yRange()[1] > breakPos[1] - 5) ) {
  ...
  collidedTop(breaker) {
    let breakPos = breaker.pos;
    if( (this.yRange()[0] > breakPos[1] + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide - 5) && (this.yRange()[0] < breakPos[1] + 5 + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide ) ) {
      let breakerLeft = breakPos[0];
      let breakerRight = breakPos[0] + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide;
  ...
  collidedLeft(breaker) {
    let breakPos = breaker.pos;
    if( (this.xRange()[0] > breakPos[0] + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide -5) && (this.xRange()[0] < breakPos[0] + 5 + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide)) {
      let breakerTop = breakPos[1];
      let breakerBot = breakPos[1] + __WEBPACK_IMPORTED_MODULE_0__util_js__["b" /* defaults */].breakerSide;
