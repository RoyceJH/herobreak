...
class Monster {
  ...
  collidedTop(breaker) {
    let breakPos = breaker.pos;
    if( (this.yRange()[0] > breakPos[1] + Util.defaults.breakerSide) && (this.yRange()[0] < breakPos[1] + 5 + Util.defaults.breakerSide ) ) {
      let breakerLeft = breakPos[0];
      let breakerRight = breakPos[0] + Util.defaults.breakerSide;
  ...
  collidedLeft(breaker) {
    let breakPos = breaker.pos;
    if( (this.xRange()[0] > breakPos[0] + Util.defaults.breakerSide) && (this.xRange()[0] < breakPos[0] + 5 + Util.defaults.breakerSide)) {
      let breakerTop = breakPos[1];
      let breakerBot = breakPos[1] + Util.defaults.breakerSide;
