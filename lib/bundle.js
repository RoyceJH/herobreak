...
class Game {
  ...
  checkCollisions() {
    this.currentAttackers.forEach(breaker => {
      ...
      if(breaker.pos[0] > this.DIM_X - __WEBPACK_IMPORTED_MODULE_5__util_js__["b" /* defaults */].breakerSide - 3) {
        hitRight = true;
      }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__boss_js__ = __webpack_require__(10);
...
class Breaker {
  ...
  move(timeDelta) {
    const velocityScale = timeDelta / NORMAL_TIME;
...
class Monster {
  ...
  collidedBottom(breaker) {
    ...
    if( (this.yRange(side)[1] < breakPos[1]) && (this.yRange(side)[1] > breakPos[1] - 7) ) {
  ...
  collidedRight(breaker) {
    ...
    if( (this.xRange(side)[1] < breakPos[0]) && (this.xRange(side)[1] > breakPos[0] - 7)) {
