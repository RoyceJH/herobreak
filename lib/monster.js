...
class Monster {
  ...
  xRange() {
    ...
    const rightBound = this.pos[0] + Util.defaults.side + 7;
    return [leftBound, rightBound];
  }

  yRange() {
    const topBound = this.pos[1] + 7;
    const bottomBound = this.pos[1]+ Util.defaults.side + 7;
  ...
  collidedTop(breaker) {
    let breakPos = breaker.pos;
    if( (this.yRange()[0] > breakPos[1] + Util.defaults.breakerSide - 5) && (this.yRange()[0] < breakPos[1] + 5 + Util.defaults.breakerSide ) ) {
      let breakerLeft = breakPos[0];
      let breakerRight = breakPos[0] + Util.defaults.breakerSide;
  ...
  collidedLeft(breaker) {
    let breakPos = breaker.pos;
    if( (this.xRange()[0] > breakPos[0] + Util.defaults.breakerSide -5) && (this.xRange()[0] < breakPos[0] + 5 + Util.defaults.breakerSide)) {
      let breakerTop = breakPos[1];
      let breakerBot = breakPos[1] + Util.defaults.breakerSide;
